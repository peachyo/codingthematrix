...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat(A.D, {k:(getitem(A,k)+getitem(B,k)) for k in {(x,y) for x in A.D[0] for y in A.D[1]}})

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D, {k:getitem(M,k)*alpha for k in M.f.keys()})

def equal(A, B):
    "Returns true iff A is equal to B"
    assert A.D == B.D
    return True if sum([getitem(A, k)==getitem(B,k) for k in {(x,y) for x in A.D[0] for y in A.D[1]}])==(len(A.D[0])*len(A.D[1])) else False

def transpose(M):
    "Returns the transpose of M"

    return Mat((M.D[1],M.D[0]), {(k[1],k[0]):getitem(M,k) for k in M.f.keys()})
